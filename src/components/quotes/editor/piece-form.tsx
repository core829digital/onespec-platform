    ...
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-3">
        <div>
          ...
          <input type="number" inputMode="numeric" step={10} min={200} max={6000} value={item.width} onChange={(e) => onWidth(Number(e.target.value) || 0)} className={`${field} font-mono`} />
        </div>
        <div>
          <label className={label}>{t("height")}</label>
          <input type="number" inputMode="numeric" step={10} min={200} max={6000} value={item.height} onChange={(e) => onHeight(Number(e.target.value) || 0)} className={`${field} font-mono`} />
        </div>
        <div>
          <label className={label}>{t("quantity")}</label>
          <input type="number" inputMode="numeric" min={1} max={50} value={item.quantity} onChange={(e) => onPatch({ quantity: Math.min(50, Math.max(1, Number(e.target.value) || 1)) })} className={`${field} font-mono`} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <div>
          ...
          <select value={item.material} onChange={(e) => onMaterial(e.target.value)} className={field}>
        ...
        <div>
          ...
          <select value={item.quality[item.material] ?? ""} onChange={(e) => onPatch({ quality: { ...item.quality, [item.material]: e.target.value } })} className={field}>
        ...
        {choices.profiles.length > 0 ? (
          <div className="sm:col-span-2">
            <label className={label}>{t("profile")}</label>
            <select value={item.profileSystem ?? ""} onChange={(e) => onPatch({ profileSystem: e.target.value || undefined })} className={field}>
              {tabs.length > 0
                ? tabs.map((g) => (
        ...
        <div>
          <label className={label}>{t("glazing")}</label>
          <select value={item.glazing} onChange={(e) => onPatch({ glazing: e.target.value })} className={field}>
        ...
        <div>
          <label className={label}>{t("finish")}</label>
          <div className="flex items-center gap-2">
            <select value={item.color} onChange={(e) => onPatch({ color: e.target.value })} className={field}>
      ...
      <details className="rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)]" open={anyAccessory}>
        ...
        <div className="space-y-3 border-t border-[var(--color-border)] p-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={label}>{t("accessoryWidth")}</label>
              <input type="number" step={10} min={100} max={6000} value={acc.width ?? ""} placeholder={String(item.width)} onChange={(e) => setAcc({ width: e.target.value ? Number(e.target.value) : undefined })} className={`${field} font-mono`} />
            </div>
            <div>
              <label className={label}>{t("accessoryHeight")}</label>
              <input type="number" step={10} min={100} max={6000} value={acc.height ?? ""} placeholder={String(item.height)} onChange={(e) => setAcc({ height: e.target.value ? Number(e.target.value) : undefined })} className={`${field} font-mono`} />
            </div>
          </div>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {choices.accessories.filter((g) => g.options.length > 0).map((g) => (
              <div key={g.category}>
                <label className={label}>{ACCESSORY_CATEGORY_LABELS[g.category as AccessoryCategory][locale] ?? ACCESSORY_CATEGORY_LABELS[g.category as AccessoryCategory].it}</label>
                <select value={acc[g.category as AccessoryCategory] ?? "none"} onChange={(e) => setAcc({ [g.category]: e.target.value === "none" ? undefined : e.target.value })} className={field}>
      ...
      <div>
        ...
        <textarea value={item.notes ?? ""} onChange={(e) => onPatch({ notes: e.target.value })} rows={2} maxLength={500} placeholder={t("notesPlaceholder")} className={`${field} resize-y`} />
      </div>
    </div>
  );
}
