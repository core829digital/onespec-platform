...
export const fanOutToTenant = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const titleFor = (type: NotifType, d: any): string => {
      switch (type) {
    ...
    const entityId: string | undefined = args.data?.quoteId ?? args.data?.configuratorId;
    ...
    for (const m of memberships) {
      await ctx.db.insert("notifications", {
        ...
        userId: m.userId,
        type: args.type,
        title: titleFor(args.type as NotifType, args.data),
        body: undefined,
        href: args.href,
      ...
      if (args.emailTemplate) {
        ...
        if (user?.email) {
          await ctx.scheduler.runAfter(0, internal.email.send, {
            template: args.emailTemplate as any,
            to: user.email,
