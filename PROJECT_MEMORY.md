...
**In corso (background agent)**: audit sicurezza completo — isolamento tenant su ogni query/mutation con `tenantId` (verifica `requireMembership` reale, non solo `requireUser`), sicurezza delle rotte pubbliche token/PIN/publicId (`/w/`, `/c/`, `/f/`, `/i/`, `/k/`), rate-limit sul PIN ospite cantieri (bug noto non confermato risolto), scan segreti/injection nelle chiamate `fetch` di `convex/billing.ts`, e ogni **errore** (non warning) di `npx eslint convex src tests`. Risultati da integrare al rientro.
