...
### 9.12 FASE 12 — chiusura batch e verifica sistematica (2026-09-22, notte)

**Verificato punto per punto** (non solo letto, testato/letto il codice reale):
- **Punto 1 (kanban cantieri)**: drag&drop **funziona davvero** — `handleDragEnd` chiama `updateCantiere({status})` end-to-end; `deleteCantiere` e' un vero hard-delete (righe + task collegati), non un bug. Tolta la doppia `requireTenantRole` ridondante (stesso fix gia' fatto su `deleteClient`).
- **Punto 4 (header sticky)**: confermato NON coerente con la sidebar (bar piatta bordo-a-bordo vs card fluttuante vetro/blur della sidebar) — **corretto**: `Topbar` ora e' `sticky top-0`, `rounded-2xl`, `bg-[var(--color-bg-alt)]/70`, `backdrop-blur-xl`, stessa ombra e margine della sidebar.
- **Punto 6 (redirect/middleware)**: `redirect-validator.ts` verificato riga per riga — allow-list per prefisso path (non dominio, perche' i redirect interni sono sempre path-relativi), blocca protocollo non-http(s), host esterno, `//host` protocol-relative, **e backslash-normalizzato a `//`** (il parser URL WHATWG normalizza `\` in `/` per schemi http, quindi `/\evil.com` diventa `//evil.com` e viene bloccato dallo stesso controllo). Varianti encoded (`%2F%2F...`) non matchano nessun prefisso della allow-list quindi falliscono comunque (l'allow-list positiva batte l'enumerare ogni encoding). Collegato a tutte le 5 pagine auth (login/register/forgot-password/reset-password/verify). Matcher middleware (`src/proxy.ts`) e' un catch-all (`/((?!_next|_vercel|.*\..*).*)`) meno gli asset statici — copre ogni rotta, incluso `/api/*` che poi passa dritto. Autorizzazione server-side indipendente dal middleware: **gia' confermata nell'audit di sicurezza precedente** (9.10/audit agent) — ogni query/mutation Convex con `tenantId` chiama `requireMembership`/`requireTenantRole` reale.
- **Punto 7 (email/delivery monitoring)**: trovato **bug reale** in `convex/http/resend_webhook.ts` — la verifica firma webhook era un HMAC-SHA256 esadecimale generico sul body, con header `resend-signature`; Resend firma tramite **Svix**, non cosi': header veri `svix-id`/`svix-timestamp`/`svix-signature`, contenuto firmato `${id}.${timestamp}.${rawBody}`, segreto e firma in **base64** (non hex), piu' guardia anti-replay sul timestamp. Con l'implementazione originale la verifica non avrebbe MAI accettato una chiamata reale da Resend (feature "delivery monitoring" rotta silenziosamente). Riscritta secondo lo schema Svix reale (fonti: docs.svix.com, resend.com/docs/dashboard/webhooks/verify-webhooks-requests). Endpoint registrato: `/api/email/webhook` su `https://spotted-basilisk-866.convex.site` (dominio `.convex.site`, non `.convex.cloud`). **Serve ancora dall'utente**: creare il webhook nella dashboard Resend puntando a quell'URL, copiare il signing secret e impostarlo come `RESEND_WEBHOOK_SECRET` su Convex prod (senza quel secret la verifica firma e' bypassata, non bloccante ma non protetta — noto, non urgente finche' non si attiva `RESEND_MODE=live`).
- **Legal pages**: create dall'altra sessione, non ancora riverificate riga-per-riga per il vincolo "non inventare" — da fare quando richiesto esplicitamente un'analisi di conformita' vera.

**Altri fix minori in questo giro**: `SortableCantiereCard.tsx` chiamava `Date.now()` nel corpo del render (violazione regola di purezza React, errore lint reale) — spostato in lazy `useState` initializer. `legal/dpa/page.tsx` apostrofo non escapato (errore lint reale). `tsconfig.json`: `apps/` escluso (ha un proprio tsconfig, altrimenti un errore WIP la' rompeva `tsc` di tutta la piattaforma). `.gitignore`: aggiunto `**/.next/` generico + `/apps/` (lo status-page va in repo separato, vedi sotto — non deve mai entrare nella history di questo repo).

**Commit `145884c` pushato** (81 file, batch completo) + **deploy Convex prod fatto** (schema additivo: tabella `emailDeliveryLog`, 2 nuovi template email, 1 nuovo indice — nessuna modifica distruttiva). Gate: tsc pulito, eslint 0 errori, vitest 254/254, build verde.
...
**Aggiornamento 2026-09-24 — superato di nuovo, questa volta da Betterstack**: l'utente ha implementato la status page con Betterstack (servizio esterno), non piu' con l'approccio custom Convex+`status.onespec.eu` descritto sopra. Su richiesta esplicita, rimosso ogni riferimento al dominio `status.onespec.eu` da questo repo (era solo nel nome di uno step del workflow `.github/workflows/notify-status-page.yml` e in `STATUS_PAGE_DEPLOYMENT.md`, ora cancellato — nessun file in questo repo reclamava davvero quel dominio, nessun CNAME/vercel.json trovato) cosi' i record DNS possono essere riassegnati senza conflitti. **Rimosso anche il workflow stesso** (`.github/workflows/notify-status-page.yml` cancellato, su richiesta esplicita) — con Betterstack a gestire la status page, non c'era piu' nessun `/api/deploy` custom da notificare. Il progetto Convex separato `onespec-status-page` resta com'e' (non toccato, decisione dell'utente se dismetterlo).
...
### 9.17 Sicurezza abbonamenti (Punto 1 parziale) + pagine legali CORE829 SRL (Punto 3) + valutazione onesta capacita' piattaforma (Punto 5) (2026-09-23/24)

Richiesta utente a 5 punti (billing anti-frode, audit colori configuratori, pagine legali con dati reali CORE829 SRL, responsive/dark mode, valutazione onesta capacita'). Eseguiti in questo giro: Punto 1 (parziale — hardening webhook, non il wizard di acquisto completo), Punto 3 (completo), Punto 5 (risposta diretta in chat, non ripetuta qui). Punti 2/4 e il resto del Punto 1 restano da fare (vedi lista "cose mancanti" sotto).

**Punto 1 — hardening webhook Stripe + chiusura falla abbonamento gratuito**:
- Verificato che firma webhook (`verifyStripeSignature`, HMAC-SHA256 con tolleranza timestamp) e idempotenza (`billingEvents` per `stripeEventId`) erano **gia' corrette** — solo verifica, nessuna modifica necessaria.
- `convex/http.ts`: URL webhook resa non-indovinabile (`/api/stripe/webhook/${STRIPE_WEBHOOK_PATH_TOKEN}`, env var — fallback a path statico finche' non impostata), + controllo soft (log, mai reject duro) contro la lista IP ufficiale Stripe fetchata dinamicamente da `stripe.com/files/ips/ips_webhooks.json` con cache 24h — **mai IP hardcoded**: prima bozza li aveva inventati, scartata e corretta prima del commit (vedi nota "non inventare" gia' applicata altrove in questa sessione).
- **Falla reale trovata e chiusa**: `registerTenant` crea ogni nuovo tenant con `planStatus:"trialing"` e nessun `trialEndsAt` — e il cron `trialSweep` scade i trial solo se `trialEndsAt` e' impostato. Risultato: un utente che si auto-registra resta per sempre su piano Base gratuito, mai scaduto, senza mai passare da Stripe. Fix in `convex/lib/enforcement.ts:enforceActivePlan`: si attiva **solo** quando `STRIPE_SECRET_KEY` e' realmente impostato (self-activating gate — comportamento di oggi invariato finche' il billing resta dormiente), e allora richiede `stripeSubscriptionId` per lo stato "trialing". Nuovo errore `SUBSCRIPTION_REQUIRED` propagato a `src/lib/errors.ts` + i18n 6 lingue. 4 test nuovi (`tests/convex/enforcement-active-plan.test.ts`), tutti verdi.
- **Non fatto**: wizard acquisto completo (onespec.eu → form obbligatorio → verifica email → redirect piattaforma → checkout con trial 14gg + verifica carta), cancellazione self-service (gia' esiste `createPortalSession`? da verificare), enforcement "niente upgrade di tier senza nuovo acquisto". Serve prima capire se `onespec.eu` (sito marketing) vive in questo repo o in un repo separato non accessibile da qui.

**Punto 3 — pagine legali con dati reali CORE829 SRL** (`src/content/legal.ts` + `legal-values.ts`, commit `d7754f1` + `a45d550`):
- Compilati come fatti verificati (non placeholder): ragione sociale "CORE829 SRL", indirizzo Str. Mihai Eminescu 10 Roman România, CUI/CIF RO54616345, Reg. com. J2026029428009, entrambi i numeri di telefono (aggiunta nuova frase in `IDENTITY_INTRO`, non esisteva placeholder telefono prima), PEC = "non applicabile" esplicito (entita' romena, PEC e' istituto italiano), DPO = "non è" stato nominato (fatto vero, non un placeholder vuoto).
- Sezione certificazioni: da placeholder-vuoto-che-sembra-incompleto a dichiarazione esplicita "nessuna certificazione posseduta" — la differenza conta legalmente (dichiarazione vera vs sezione mancante).
- Aggiunta una sezione "Contatti per questioni legali e contrattuali" (office@core829.net) in fondo a **tutti e 7** i documenti legali (privacy, termini-di-servizio, termini-di-utilizzo, cookie, gdpr, sicurezza, qualita), per istruzione esplicita dell'utente che quell'indirizzo deve comparire su ogni pagina legale. `hello@onespec.eu` lasciato invariato (unica email OneSpec, per istruzione esplicita).
- Retention account: allineato al vero grace period di 30gg in `convex/account.ts:requestDeletion` (non un numero a caso). Retention preventivi/audit-log: riscritta la frase per dire onestamente che **non esiste cancellazione automatica programmata** (vero, nessun cron di retention esiste) invece di lasciare un placeholder che suggeriva un numero in arrivo.
- Regione hosting Convex: confermata dall'utente = Stati Uniti — piattaforma venduta/usata in UE quindi e' un vero trasferimento verso paese terzo, gia' coperto dalla frase SCC esistente.
- **Lasciati intenzionalmente vuoti** (decisioni di business, non fatti da dedurre): legge applicabile e foro competente per i Termini di servizio.

**Punto 5 — risposta onesta capacita'/completezza**: data per intero in chat (non file), sintesi: query non paginate su tabelle che crescono (audit log, notifiche) sono il rischio di degrado piu' concreto oltre qualche decina di tenant attivi da tempo, non un crash improvviso; rate limiting sistemico solo parziale sugli endpoint pubblici; ~30 funzioni Convex mai collegate a UI, PDF ancora via `window.print()` su alcune pagine (verificato pero' che `quotes/[id]/print` usa gia' `pdf().toBlob()` pulito, non tutte le pagine sono nella stessa situazione — da riverificare per pagina), i18n 18 pagine ancora hardcoded IT, sistema Alpha da migrare se si passa a piano abbonamento semplice.

**Cose mancanti confermate col codice, da aggiungere alla lista task (non ancora fatte)**:
1. **Wizard di acquisto abbonamento completo** (Punto 1, resto) — form obbligatorio onespec.eu, verifica email, redirect a checkout con trial 14gg + verifica carta, enforcement "no upgrade senza nuovo acquisto". Richiede prima chiarire dove vive `onespec.eu`.
2. **Selettore colore telaio non esiste in `ShowroomWidget`** (`src/components/widget/ShowroomWidget.tsx`) — lo stato `color` era bloccato su "white" per sempre (nessun `onChange` mai wired, trovato durante il lint cleanup). `MaterialConfig` ha solo "colore ferramenta" (maniglia), non colore telaio. I 3 colori reali usati altrove nel sistema (PDF preventivo, `COLOR_LABELS` in `src/lib/pdfs/QuotePrintPDF.tsx`) sono white/anthracite/woodgrain (RAL 9016/9001, RAL 7016, effetto legno) — punto di partenza per costruire il picker quando si fa il Punto 2 (audit configuratori).
3. **`ComplianceBadges`** (`src/components/installations/ComplianceBadges.tsx`, FASE 4.6) — componente scritto (badge norma + bandiere di conformita' per mercato) ma **mai renderizzato** in `installations/page.tsx` — nessuna query carica `norm`/`flags`/`fundingTitle` per il dossier. Trovato durante il lint cleanup (import orfano).
4. **Offline queueing mancante su Installazioni** — `src/lib/offline-sync.ts` esporta `enqueue`, usato da altre pagine campo ma mai importato/chiamato in `installations/page.tsx` (solo `flushQueue`/`subscribeSyncState` sono usati li'). Creare un dossier offline non viene messo in coda per la sync.
5. **Toggle mensile/annuale abbonamento non esisteva** — **gia' corretto in questo giro**: `account/billing/page.tsx` aveva `cycle`/`setCycle` mai collegato a nessun controllo UI, quindi il checkout annuale (gia' supportato end-to-end dal backend, `convex/billing.ts:createCheckoutSession` con `cycle: "annual"`) era irraggiungibile. Aggiunto il toggle Mensile/Annuale (i18n gia' presente in tutte le 6 lingue, mai usato prima).
6. **Bug calcolo Uw nel PDF preventivo** — **gia' corretto in questo giro**: `estimateUw()` in `QuotePrintPDF.tsx` calcolava `uFrame` per materiale (pvc/alu/legno) ma poi usava sempre un valore fisso `1.5` nella formula finale, ignorando il materiale reale. Il documento tecnico mostrava lo stesso Uw indipendentemente dal telaio scelto.
7. **Switch bilingue Lussemburgo (FR/DE) sulla stampa preventivo non aveva effetto reale** — **gia' corretto in questo giro**: il toggle `luLang` esisteva in UI ma la lingua del documento PDF era sempre derivata dalla regione (`quote.regionCode`), mai dalla scelta dell'utente — bug preesistente, non introdotto qui. Aggiunto un prop `lang` esplicito a `QuotePrintPDF` che sovrascrive la lingua derivata quando la regione e' LU.
8. **`Pagination` component era completamente orfano** (zero import in tutto `src/`, verificato con grep) **e aveva un bug reale**: il controllo "elementi per pagina" aveva un commento letterale "We'll handle this via a custom event or context" — cioe' non faceva nulla. Corretto (ora chiama `setItemsPerPage` davvero) ma il componente resta non collegato a nessuna pagina lista — da valutare se serve ancora o va rimosso.
9. **Pagine legali nel menu**: solo 4 dei 7 documenti legali sono linkati in `legal/layout.tsx` (privacy, termini-di-servizio, dpa, cookie) — mancano termini-di-utilizzo, gdpr, sicurezza, qualita.

**Pulizia lint completa** (commit da fare in questo turno): `npx eslint` (bare, senza path — **esattamente il comando che gira in CI**, diverso da `eslint src convex` usato nei giri precedenti) passava da **342 errori + 11624 warning** a **0 errori, 0 warning** sui soli file tracciati da git. Causa radice di TUTTI gli errori: la cartella di riferimento `CONFIGURATORE NUOVO AGGIORNATO PER MONTATORI, RIVENDITORI E SHOWROOM/` (27 file tracciati, materiale di riferimento per il Punto 2, non codice applicativo) contiene bundle JS minificati vendorizzati che violano regole reali (`no-explicit-any`, `no-empty-object-type`, ecc.) — esclusa ora in `eslint.config.mjs`. Il resto erano ~90 warning reali (import/variabili inutilizzate) su ~20 file, risolti file per file — **non tutte cancellazioni pure**: alcuni erano bug veri dietro variabili "inutilizzate" (vedi punti 5/6/7/8 sopra), scoperti proprio ripulendo il lint invece di silenziarlo con `_`/eslint-disable.

**CI GitHub Actions — root cause trovata e corretta**: verificato via API GitHub (nessun `gh` CLI disponibile, usato `curl` diretto su `api.github.com/repos/.../actions/runs`) che il workflow CI **non era mai stato verde nemmeno una volta** dalla sua creazione (commit `321157f` in poi, 6 run consecutivi falliti). Causa: lo step "Lint" usa `npm run lint` = `eslint` bare (tutto il repo), mentre i controlli locali di questa sessione fino ad ora usavano `eslint src convex` (solo le due cartelle applicative) — la cartella di riferimento configuratori non veniva mai vista localmente. Confermato riproducendo l'esatto ambiente CI (checkout pulito = `apps/status-page/` locale-non-tracciato spostato temporaneamente fuori, poi ripristinato) e rilanciando `npm run typecheck && npm run lint && npm test && npm run build` — tutti e 4 verdi.
...
- **Full access verificato in produzione**: `contact.core829@gmail.com` e `office@winex.ro` hanno gia' `isPlatformAdmin: true` + `unlimitedAccess: true` (fatto dall'altra sessione, commit `8c6bbab`) — verificato query diretta su prod, 0 modifiche necessarie, solo conferma.
- **Nuovo `convex/lib/rbac.ts`**: modello centralizzato "ruolo tenant (owner>admin>member) = pavimento, entitlement del piano = soffitto" — stesso nome ruolo fa di piu' su un piano superiore. `requirePermission(ctx, tenantId, action)` unico controllo, 2 codici errore distinti (`INSUFFICIENT_ROLE` vs `PLAN_UPGRADE_REQUIRED`) cosi' il client puo' mostrare "chiedi al tuo admin" vs "fai upgrade" invece di un errore generico. Cablato come riferimento su team.invite/remove/cancelInvite + tenant.settings (updateTenant) — i restanti ~150 call site restano su `requireTenantRole` (invariato, corretto), non riscritti tutti sotto pressione di tempo. 5 test nuovi (`tests/convex/rbac.test.ts`).
- **Race condition aggiuntiva trovata e corretta**: `convex/lib/ratelimit.ts`'s `consumeToken` usava `.unique()` sul lookup del bucket — stesso pattern gia' corretto altrove in questa sessione (membership, guest PIN): due richieste concorrenti sul primo insert dello stesso `bucketKey` potevano creare una riga duplicata e far esplodere ogni richiesta futura su quel bucket. Passato a `.first()`.
- **Deploy Convex prod fatto 3 volte in questo giro** (sync codice, poi migrazione `grantFullAccessToFounders` — idempotente, 0 modifiche perche' gia' applicata dall'altra sessione — poi il modulo RBAC).

**Chiavi API ancora mancanti per il lancio** (da fornire dall'utente, nessuna azione di codice possibile senza):
1. **Cloudflare Turnstile**: `NEXT_PUBLIC_TURNSTILE_SITE_KEY` + `TURNSTILE_SECRET` — il bot-check e' scritto e dormiente (vedi FASE L-5, `src/lib/turnstile-client.ts`), serve solo accenderlo.
2. **Stripe**: `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_WEBHOOK_PATH_TOKEN` (url non-indovinabile, vedi 9.17), + i Price ID veri per ogni piano/regione/ciclo (mensile/annuale) da creare nella dashboard Stripe — `resolveStripePriceId` in `convex/billing.ts` li legge da env var, nessuno esiste ancora.
3. Nessun'altra chiave nuova identificata — Resend e' gia' live (9.20), Convex/Vercel/Sentry/PostHog gia' configurati.

**CDN/caching/Redis** (chiesto esplicitamente dall'utente, risposta diretta senza modifiche di codice): i file caricati (foto, loghi) passano gia' per `ctx.storage` di Convex, che serve via il proprio CDN — nessuna configurazione nostra necessaria. Gli asset statici Next.js (`/_next/static/*`) sono gia' cache-ati dalla CDN edge di Vercel di default, nessun header custom scritto ne' necessario. Redis non risulta necessario alla scala attuale (nessuna lettura pesante ripetuta identificata che il reactivity layer di Convex non gestisca gia' — da rivalutare solo se emergono cataloghi pubblici ad alto traffico).

**Non fatto in questo giro per onesta' sui tempi**: l'audit "ogni singolo wizard/form/endpoint" richiesto per intero dall'utente non e' stato ripetuto da zero — si e' verificato quanto gia' coperto da questa e dall'altra sessione (FASE L rate limiting, audit a11y 9.15, lint/CI 9.17, security audit endpoint 9.19) invece di rifare un audit completo sovrapposto, per non sprecare tempo su cose gia' verificate. Se serve un audit fresh mirato su una parte specifica (es. solo i webhook Stripe, o solo i form pubblici), va richiesto esplicitamente.
